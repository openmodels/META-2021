@defcomp PatternScaling begin
    ...
    function run_timestep(pp, vv, dd, tt)
        for cc in dd.country
            ...
            # end
            vv.T_country[tt, cc] = pp.ps_alpha[cc] + pp.ps_beta[cc] * pp.T_AT[tt] # (pp.GMST_2010+pp.T_AT[tt]-pp.T_AT[TimestepIndex(1)])*vv.scale_country[tt, cc]
        end
    end
end

function addPatternScaling(model)
    params = CSV.read("../data/pattern-scaling_new.csv", DataFrame) # params = CSV.read("../data/pattern-scaling.csv", DataFrame)
